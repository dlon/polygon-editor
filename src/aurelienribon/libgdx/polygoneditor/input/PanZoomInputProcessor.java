...
public class PanZoomInputProcessor extends InputAdapter {
    ...
    public void update(float dt) {
        if (Gdx.input.isKeyPressed(Keys.LEFT)) {
            canvas.camera.translate(-scrollSpeed * dt, 0f, 0f);
        ...
        if (Gdx.input.isKeyPressed(Keys.UP)) {
            canvas.camera.translate(0f, scrollSpeed * dt, 0f);
            canvas.camera.update();
        }
    ...
    /*@Override
        public boolean keyDown(int keycode) {
            if (keycode == Keys.LEFT) {
                canvas.camera.translate(-5.0f, 0f, 0f);
		canvas.camera.update();
            }
            return false;
        }*/
    ...
    public boolean touchDragged(int x, int y, int pointer) {
        ...
        canvas.camera.translate(-delta.x, -delta.y, 0);
        canvas.camera.update();
    ...
    public boolean scrolled(int amount) {
        ...
        canvas.camera.zoom = 100f / zoomLevel;
        canvas.camera.update();
        return false;
    }
}
